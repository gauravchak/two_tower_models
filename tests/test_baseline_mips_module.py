...
class TestBaselineMIPSModule(unittest.TestCase):
    ...
    def test_output_shapes(self):
        indices, mips_scores, embeddings = self.module(self.query_embedding, self.num_items)
        self.assertEqual(indices.shape, torch.Size([self.batch_size, self.num_items]))
        self.assertEqual(mips_scores.shape, torch.Size([self.batch_size, self.num_items]))
        self.assertEqual(embeddings.shape, torch.Size([self.batch_size, self.num_items, self.embedding_dim]))
